...
class SuperIndividual(object):
    def __init__(self, global_settings, diapausedepth, environment, environment_profiles, environment_index, eggmass=0.23, nindividuals=10000, genes=None, cxthreshold=0.7, muthreshold=0.2, datalogger=None, unique_id=None):
...
#all evolvable attributes range from 0 - 1 in floating point designation
        ...
        if genes is not None:
            ...
            self.genome = dotdict({g:np.random.rand(1)[0] if self.global_settings['stochastic'] else 0.2 for g in EXPECTED_GENES})
    ...
    def update_lifestage(self):
        ...
        if self.nvindividuals <= self.global_settings['virtualindividualthrehold'] or self.age >= self.global_settings['ageceiling'] or self.totalfecundity >= self.global_settings['fecundityceiling']:
            self.lifestatus = 0

    def stage_lt_2(self):
        #eggs and non-feeding naupliar stages (Egg-NII)
        self.update_vert()
        self.zpos, self.zidx = vm.verticalmigration_dsc0(smld = self.environment['mld'][self.environment_index], maxdepth=self.maxdepth, 
                                                                                    mindepth = self.mindepth, depthrange=self.global_settings['depthrange'], continuousdepth=False)

        #update the current thermal history (this is an arithmetic mean)
        self.thermalavg = (self.thermalavg + self.get_zi('temperature')) / 2.00 #!!!!!!!!!!!!!!
        ...
        currentdevelopmentaltime, self.feedingrate, currentgrowthrate, self.metabolicrate, self.egestionrate = gdm.growthdevelopmentmetabolism_dsc0(
    ...
    def stage_3_9(self):
        ...
        self.feedingrate, currentgrowthrate, self.egestionrate, self.metabolicrate = gdm.growthanddevelopment_dsc1(temperature = self.get_zi('temperature'),
    ...
    def stage_12(self):
        ...
        if self.sex == "M":
            ...
            currentgrowthrate = self.apply_dsc3_male_growthdevel(maxdistance=maxdistance, traveldistance=traveldistance)
            ...
            if currentgrowthrate >= 0.00:
                if self.structuralmass < self.adultsize:
                    if self.inseminationstate == 1:
                        self.structuralmass += currentgrowthrate * (1.00 - self.genome.a5_energyallocation)
                        self.reproductiveallocation = currentgrowthrate * self.genome.a5_energyallocation
    ...
    def diapause0(self):
        ...
        if self.diapausestrategy == -1:
            ...
            self.diapausestrategy = 0 if diapausern <= self.genome.a6_diapauseprobability else 1
            ...
            if self.diapausestrategy == 0:
                ...
                if self.reservemass / self.structuralmass >= 1.00:
                    ...
                    # Moved max structural mass to end of if clause as occured in both cases 
                     ...
                     if self.structuralmass < self.global_settings['energyallocthreshold1']:
                         currentenergyallocation = 0.00
                     elif self.structuralmass >= self.global_settings['energyallocthreshold1'] and self.structuralmass <= self.global_settings['energyallocthreshold2']:
                         currentenergyallocation = currentgrowthrate * self.genome.a5_energyallocation * 1.00 / (1.00 + math.exp((60.00 - self.structuralmass) / 20.00))
                     else:
                         currentenergyallocation = currentgrowthrate * self.genome.a5_energyallocation
                     #end if

                     self.reservemass = self.reservemass + currentenergyallocation
                     self.structuralmass = self.structuralmass + currentgrowthrate - currentenergyallocation
                ...
                if self.structuralmass >= self.adultsize:
                    ...
                    if self.structuralmass < self.global_settings['energyallocthreshold1']:
                        ...
                        currentenergyallocation = currentgrowthrate * 1.00 / (1.00 + math.exp((60.00 - self.structuralmass) / 20.00))
                    else:
        ...
        if self.diapausestrategy == 0:
            ...
            if self.reservemass / self.structuralmass >= self.genome.a7_diapauseentry:
                ...
                if self.maxdepth >= self.global_settings['diapausedepththreshold1']:
                    ...
                    self.diapausedepth, self.diapausedepthidx = vm.diapausedepthselection(maxdepth = self.maxdepth,
                    ...
                    if self.datalogger is not None:
                        ...
                        diapauselogidx = 0 if currentdevelopmentalstage == 10 else 1
                        #this logs diapause entry data of civ stages of true diapause
                        self.datalogger.add_den(self.get_log_data(), diapauselogidx, self.diapausemode)
                    #end if 

                elif self.maxdepth < self.global_settings['diapausedepththreshold1'] and self.maxdepth >= self.global_settings['diapausedepththreshold0']:
    ...
    def diapause1(self):
        ...
        if self.diapausemode == 0:
            currentdiapausemetabolicrateadj = self.global_settings['diapausemetabolicrateadj0']
        else:
            currentdiapausemetabolicrateadj = self.global_settings['diapausemetabolicrateadj1']

        self.feedingrate, currentgrowthrate, self.egestionrate, self.metabolicrate = gdm.growthanddevelopment_dsc2_diapause(temperature = self.get_zi('temperature'),
                                                                                                                            strmass = self.structuralmass,
        ...
        if (1.00 - self.reservemass / self.reservesatdiapauseentry) >= self.genome['a8_diapauseexit']:
    ...
    def apply_dsc1_verticalmigration(self):
        self.zpos, self.zidx, maxdistance, traveldistance = vm.verticalmigration_dsc1(temprange = self.get_profile('temperature'),
    ...
    def apply_dsc2_verticalmigration(self):        
        self.zpos, self.zidx, maxdistance, traveldistance = vm.verticalmigration_dsc2(temprange = self.get_profile('temperature'),
                                                                                                        fconrange = self.get_profile('food1concentration'),
                                                                                                        iradrange = self.get_profile('irradiance'),
                                                                                                        maxirad = self.global_settings['maxirradiance'],
                                                                                                        visualpredatordensityrange = self.get_profile('pred1dens'),
    ...
    def apply_dsc2_growthdevel(self, maxdistance, traveldistance):
        self.feedingrate, currentgrowthrate, self.egestionrate, self.metabolicrate = gdm.growthanddevelopment_dsc2(temperature = self.get_zi('temperature'),
                                                                                                                    foodcon = self.get_zi('food1concentration'),
                                                                                                                    strmass = self.structuralmass,
                                                                                                                    resmass = self.reservemass,
                                                                                                                    maxzd = maxdistance,
    ...
    def apply_dsc3_male_growthdevel(self, maxdistance, traveldistance):
        self.feedingrate, currentgrowthrate, self.egestionrate, self.metabolicrate = gdm.growthanddevelopment_dsc3_male(temperature = self.get_zi('temperature'),
                                                                                                                          strmass = self.structuralmass,
                                                                                                                          resmass = self.reservemass,
                                                                                                                          maxzd= maxdistance,
                                                                                                                          actzd = traveldistance,
    ...
    def apply_dsc0_mortality(self):
        currentmortalityrisk = sv.mortalityrisk_dsc0(strmass = self.structuralmass,
                                                             maxstrmass = self.maxstructuralmass,
    ...
    def apply_dsc2_mortality(self):
        currentmortalityrisk = sv.mortalityrisk_dsc2(strmass = self.structuralmass,
                                                     maxstrmass = self.maxstructuralmass,
                                                     resmass = self.reservemass,
                                                     irradiance = self.get_zi('irradiance'),
                                                     maxirradiance = self.global_settings['maxirradiance'],
    ...
    def get_currentcmm(self):
       return self.global_settings['cmm_lower'][self.developmentalstage] + (self.global_settings['cmm_upper'][self.developmentalstage] - self.global_settings['cmm_lower'][self.developmentalstage]) * self.genome.a1_bodysize
    ...
    def get_spatial_log_data(self, varlist):
        ...
        carbon_adj = ['structuralmass', 'reservemass', 'feedingrate', 'egestionrate', 'metabolicrate']
    ...
    def get_log_data(self):
        return {'individuals':self.nvindividuals, 'structuralmass':(self.structuralmass * self.tnvindividuals) / 1e6, 'reservemass':(self.structuralmass * self.nvindividuals) / 1e6}
